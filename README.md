...
1. What are the main differences between a stateful and a functional component?
   -Functional components are simply JavaScript functions. Before the advent of hooks in React, they were mostly referred to as stateless or presentational components because then they only accepted and returned data to be rendered to the DOM.
   -The class component, a stateful/container component, is a regular ES6 class that extends the component class of the React library. It is called a stateful component because it controls how the state changes and the implementation of the component logic.

2. When does a componentWillMount function be called? What about a componentWillUpdate?
   -componentWillMount() is invoked immediately before mounting occurs. It is called before render(), therefore setting state in this method will not trigger a re-rendering. componentWillUpdate() is called every time a re-render is required, such as when this.setState() is called.
...
4. What are the three step of creating a successful test? What is done in each phase?
   -Arrange- Set up, decide what the interface should be.
   -Act- Do something, "query" the DOM for a specific node by its text that we expect to see in the browser.
   -Assert- Make sure that what you expected to happen is actually happening
